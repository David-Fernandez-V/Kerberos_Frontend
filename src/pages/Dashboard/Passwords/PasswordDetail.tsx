          ...
          <form id="passwordForm">
            ...
            <Feature title='Credenciales de inicio de sesión'>
              <FormControl id="user">
                ...
                <InputGroup>
                  <Input type="text" value={passwordDetail?.username} isReadOnly/>
                  <InputRightElement width="4.5rem">
                    <Tooltip label="Copiar usuario">
                      <IconButton aria-label="Copiar" ml={3} h="1.75rem" onClick={() => copy(passwordDetail ? passwordDetail.username : "")}>
                        <FiCopy/>
                      </IconButton >
                    </Tooltip>
                  </InputRightElement>
              ...
              <FormControl id="password">
                ...
                <InputGroup>
                  ...
                  <InputRightElement width="4.5rem">
                    <IconButton aria-label="Cambiar visibilidad" variant='ghost' h="1.75rem" onClick={() => setShowPassword(!showPassword)}>
                      {showPassword ? <FaEyeSlash/> : <FaEye/>}
                    </IconButton >
                    <Tooltip label="Copiar contraseña">
                      <IconButton aria-label="Copiar" mr={7} h="1.75rem" onClick={() => copy(passwordDetail ? passwordDetail.password : "")}>
                        <FiCopy/>
                      </IconButton >
                    </Tooltip>
                  </InputRightElement>
            ...
            <Feature title='Historial'>
              <HStack>
                <Text fontWeight="bold">Ultima modificación:</Text><Text>{passwordDetail && new Date(passwordDetail.updated_at).toLocaleString()}</Text>
              </HStack>
              <HStack>
                <Text fontWeight="bold">Creado: </Text><Text>{passwordDetail && new Date(passwordDetail.created_at).toLocaleString()}</Text>
              </HStack>
            </Feature>
          </form>
        </ModalBody>
        ...
        <ModalFooter >
          <Flex w='100%'>
            <Tooltip label='Eliminar sesión'>
              <IconButton ml={5} mr={2} aria-label='Borrar'  variant='ghost' color='red.600'>
