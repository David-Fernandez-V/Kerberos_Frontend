...
function FolderForm({isOpen, onClose}: Props) {
    ...
    const { register, handleSubmit, formState: { errors }, setError, reset } = useForm<folderForm>({
                ...
                <ModalBody>
                    <form id="folderForm" onSubmit={handleSubmit(onSubmit)}>
                        <FormControl id="name">
                            ...
                            {errors?.name?.message && (<Text color={"red.600"}>{errors?.name?.message}</Text>)}
                        </FormControl>
                    </form>
                </ModalBody>
                <ModalFooter>
                    <Button form="folderForm" type="submit" mr={3} >Guardar</Button>
