...
function NoteForm({isOpen, onClose}: Props) {
    ...
    const { register, handleSubmit, formState: { errors }, setError, reset } = useForm<noteForm>({
    ...
    return (
        <Modal  isOpen={isOpen} onClose={onClose} size="xl" scrollBehavior='inside'>
            ...
            <ModalContent>
                  ...
                  <form id="noteForm" onSubmit={handleSubmit(onSubmit)}>
                      <Feature title="Nota">
                        <FormControl id="title">
                          <Input variant="flushed" placeholder='Título' type="text" {...register("title")}/>
                          {errors?.title?.message && (<Text color={"red.600"}>{errors?.title?.message}</Text>)}
                        </FormControl>
                        <FormControl id="notes" mt={6}>
                          ...
                          <Textarea 
                            ...
                            {...register("content")} 
                          />
                          {errors?.content?.message && (<Text color={"red.600"}>{errors?.content?.message}</Text>)}
                        </FormControl>
                      </Feature><br />
                      {/*Opciones extra */}
                ...
                <Divider/>
                <ModalFooter>
                  <Button form="noteForm" type="submit" mr={3} >Guardar</Button>
